...
export class amazelogo {
    constructor(params) {
        var me = this;
        this.idCont = params.idCont;
        this.inSVG = params.inSVG ? params.inSVG : false; 
        this.cont = params.cont ? params.cont : d3.select("#"+params.idCont);
        this.textes = params.textes ? params.textes : false;
        ...
        this.init = function () {
            ...
            this.cont.select('#'+idLogo).remove();
            //ajoute le nouveau logo
            if(me.inSVG){
                svg = this.cont.append("g").attr("id", idLogo);
            }else{
            ...
            if(me.posis)global.attr('transform','translate('+me.posis.x+" "+me.posis.y+')');            
        ...
        this.getMaze = function(nbCol,nbRow){
            scaleW = d3.scaleLinear().domain([0, nbCol]).range([0, me.width])
            scaleH = d3.scaleLinear().domain([0, nbRow]).range([0, me.height])
            let pp =  displayMurs(maze(nbRow,nbCol));
            ...
            //dessine les points du labyrinte en svg
            let aleaOuvre =  d3.randomUniform(0, 100), 
                murs = global.append('g').attr('id',idLogo+'murs').selectAll('.'+idLogo+'mur').data(pp.murs).enter().append('path')
                .attr("class", idLogo+"mur")
                .attr("id", (d,i)=>idLogo+"mur"+i)
                .style("stroke", (d,i)=>{
                    let c = me.colorMur == 'alea' ? color(aleaColor()) : me.colorMur
                    //ouvre des murs au nord
                    if(i<nbCol && aleaOuvre()<50)c='none';
                    //ouvre de mur au sud
                    if(i>pp.murs.length-nbCol && aleaOuvre()<50)c='none';                    
                    return c;
                })
                .style("stroke-width",me.margin/3)
                .style("transform-origin", "0 0")
