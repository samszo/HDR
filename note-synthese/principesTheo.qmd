...
Nous montrerons dans le chapitre suivant comment ces processus peuvent s'appuyer sur une cartographie des connaissances @sec-principesCarto, mais décrivons maintenant plus en détail les principes théoriques que nous définissons pour modéliser ces intelligences collectives dans des écosystèmes des connaissances.

## Nos 8 principes fondamentaux d'information-communication {#sec-principesInfoCom}
...
Notre hypothèse principale que nous détaillerons plus loin @sec-semioGenerique , est qu'il est possible de modéliser ces connaissances en formalisant les pliages et leurs dynamismes dans trois directions correspondant à **trois pouvoirs fondamentaux** :
...
> « Le point de vue d’une éthique c’est : de quoi es-tu capable, qu’est-ce que tu peux ? »[^prinTheo3]

[^prinTheo3]: Cours de Deleuze du 09/12/1980 http://www2.univ-paris8.fr/deleuze/article.php3?id_article=137
...
Les illusions que le numérique procure, tendent à nous faire croire à la dématérialisation en simulant par exemple des univers immersifs où nous vivons avec des avatars de toutes sortes [@amato2013] d'autres actualités que celles de notre corps. Mais en dernière instance, nous nous positionnons en tant que matérialiste [@comte-sponville1998] : nous sommes matière et nous évoluons dans des espaces matériels.
...
Contrairement à Gärdenfors, le langage IEML propose un « filet topologique » [@lévy2011, p. 257] dont les espaces métriques sont purement conceptuels puisque les rapports de voisinage sont définis suivant six concepts (être, signe, chose, actuel, virtuel, vide) associés à trois positions conceptuelles (substance, attribut, mode) sur six couches. Il en résulte une grille topologique très vaste : (6\*6\*6)^6^ soit 1,015599567×10¹⁴ positions possibles. Une infime partie de ces positions (3418[^prinTheo6]) ont été interprétées, classifiées et référencées par Pierre Lévy et ses équipes pour donner du sens à cette topologie et fournir un vocabulaire de base utilisable avec un éditeur dédié à ce langage[^prinTheo7]. Cette solution de cartographie des espaces conceptuels est élégante et très ambitieuse, mais elle se confronte à plusieurs difficultés.

[^prinTheo6]: Le chiffre correspond au nombre d'adresses définies dans le dictionnaire IEML à la date du 25/01/2023. Ce travail toujours en train de se faire est consultable ici : https://github.com/plevyieml/ieml-language/

[^prinTheo7]: L'éditeur IEML était accessible (https://dev.intlekt.io/), mais n'est plus accessible
...
### Troisième dimension : les actants comme connaissances des dynamiques génératives entre topos et chôra {#sec-dimensionActant}
...
En résumé, le diagramme suivant @fig-globalMetrique que nous avons publié dans [@szoniecky2018], présente les dimensions de l'existence (physique, actant, concept, rapport) corrélées aux métriques (euclidienne, topographique, topologique, temporelle) permettant de les mesurer, aux concepts prétopologiques (extérieur, bord, intérieur, flux) permettant de les modéliser et aux analogies (branches, graines, racines, sève) permettant de les représenter :
...
### Calculer la complexité existentielle {#sec-calculerComplex}
...
-   infos
    ...
    -   resources[^prinTheo12] : liste des ressources impliquée dans le calcul avec les informations :
...
[^prinTheo12]: Cette information n'est présente que dans les calculs via l'API, mais pas avec le module Export

[^prinTheo13]: Cette information n'est présente que pour les ressources de niveau 1 dans les calculs via l'API, mais pas avec le module Export
...
### Représenter la complexité existentielle {#sec-representerComplex}
...
Le dernier calcul de complexité donne un total de 265 qui se décompose comme le montre la figure suivante @fig-detailCalculComplexity :

![Détail du calcul de complexité](images/localhost_samszo_omk_admin_item_69442.png){#fig-detailCalculComplexity}

Cette complexité peut se représenter par le diagramme suivant @fig-ComplexityCita qui reprend le modèle général présenté plus haut @fig-globalMetrique et dont les principes cartographiques seront décrits en détail dans le chapitre suivant @sec-principesCarto :
...
### Naviguer dans les résultats {#sec-naviguerComplex}
...
Plus globalement, il est possible de voir l'intégralité des éléments composants l'écosystème pour voir la répartition des existences suivant leur dimension et leur complexité comme nous l'avons montré dans l'analyse de notre écosystème de connaissances @sec-dataEcoCon

### Analyser la complexité des existences {#sec-analyserComplex}

La complexité existentielle obtenue par les calculs que nous venons de présenter est un indicateur très utile pour analyser un écosystème de connaissances. Par exemple, nous pouvons enrichir nos analyses des personnes participant à notre écosystème @sec-personneEcoCon en prenant en compte la complexité existentielle de ces personnes qui va de 7818 à 11 et qui donne un classement de leur importance par un simple tri sur cet indicateur.

Par exemple, si on examine le tableau des 20 personnes dont la complexité est la plus grande @tbl-personComplexityMax, on retrouve par ordre décroissant de complexité :
...
: Personnes ayant la plus grande complexité dans notre écosystème de connaissances {#tbl-personComplexityMax}
...
Nous avons déjà présenté @sec-principesInfoCom le cycle de la sémiose comme quatrième principe fondamental de la modélisation des connaissances. Revenons, plus en détail sur ce principe pour expliquer la production du sens proposée par [@µ2015b]. L’intérêt de cette proposition est de décrire la production du sens comme un cycle continu de relations dans un contexte produisant trois processus : anasémiose, élaboration sémiotique et catasémiose.
...
### L’écosystème du spectacle vivant {#sec-ecoSpectacle}
...
### Pouvoir d’agir d’un dispositif numérique {#sec-pouvoirDispositif}
