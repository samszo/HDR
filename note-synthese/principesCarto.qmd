...
Dans notre cas, pour concevoir des cartes géographiques en deux dimensions nous utilisons des librairies JavaScript Open Source comme leaflet.js[^principescarto-19] ou D3.js qui permettent de manipuler des données géographiques modéliser avec le format GeoJSON[^principescarto-20]. Voici par exemple la représentation géographique de mes collaborations dans le monde à partir de mes dépôts dans HAL[^principescarto-21] :
...
Nous nous sommes inspirés des travaux d'Amit Patel[^principescarto-26] pour mettre en place une grille hexagonale et les fonctionnalités nécessaires pour les applications prétopologiques que nous avons codées dans une librairie JavaScript[^principescarto-27] et mises en application dans un module Omeka S  afin de gérer les manipulations d'informations dans une base de données.
