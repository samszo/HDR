...
[^principescarto-1]: cf. <https://samszo.univ-paris8.fr/conf_errance/cours_systeme-information-programation-internet/slide.html?diapo=5>
...
-   4 modules spécifiques pour une gestion spécifique des données dans Omeka S:
    ...
    -   JDC : ce module[^principescarto-11] fournit les interfaces nécessaires pour modéliser un écosystème de connaissances suivant une ontologie éthique @sec-modeleOntoEthique.
...
C'est pourquoi nous avons choisi de travailler dans un environnement Web afin de créer dynamiquement des graphiques à partir d'un flux de données et surtout de rendre ces graphiques interactifs. L'autre choix important que nous avons fait est d'utiliser le langage graphique SVG[^principescarto-15] qui permet de manipuler chaque composant graphique de manière autonome [@fry2008]. Ainsi les points, les lignes et les plans disposent d'une autonomie en termes de propriétés graphiques, événementielles et informationnelles. Grâce à la librairie JavaScript D3.js[^principescarto-16] (Data Driven Document) nous pouvons gérer ces propriétés en pilotant les graphiques à partir des données ou à l'inverse les données à partir des graphiques.

[^principescarto-15]: <https://developer.mozilla.org/en-US/docs/Web/SVG>

[^principescarto-16]: [https://observablehq.com/\@d3/gallery?utm_source=d3js-org&utm_medium=hero&utm_campaign=try-observable#maps](https://observablehq.com/@d3/gallery?utm_source=d3js-org&utm_medium=hero&utm_campaign=try-observable#maps){.uri}
...
Pour réfléchir sur ces informations les humains ont depuis longtemps développé des systèmes de représentation que ce soit pour le temps [@rosenberg2013; @domenget2017], l'espace [@béguin2017] ou la combinaison des deux [@serres1997; @giacona2019; @aït-touati2019]. Nous ne rentrerons pas ici dans l'analyse de ces représentations cela dépasserais de loin notre propos qui est de présenter nos principes cartographiques. Nous renvoyons le lecteur curieux à la veille que nous faisons depuis plus de dix ans sur cette question[^principescarto-17].

[^principescarto-17]: <https://www.diigo.com/user/luckysemiosis?query=%23spatiotempo>
...
: Exemples de projections géographiques[^principescarto-18] {#tbl-projGeo}

[^principescarto-18]: <https://github.com/d3/d3-geo-projection>

Dans notre cas, pour concevoir des cartes géographiques en deux dimensions nous utilisons des librairies JavaScript Open Source comme leaflet.js[^principescarto-19] ou D3.js qui permettent de manipuler des données géographiques modéliser avec le format GeoJSON[^principescarto-20]. Voici par exemple la représentation géographique de mes collaborations dans le monde à partir de mes dépôts dans HAL[^principescarto-21] @sec-datavizActiChercheur :

[^principescarto-19]: <https://leafletjs.com/>

[^principescarto-20]: <https://fr.wikipedia.org/wiki/GeoJSON>

[^principescarto-21]: <https://samszo.github.io/StatsHAL/world.html?q=Samuel%20Szoniecky>
...
Nous suivons sur ce point les principes spinozistes d'une modélisation ontologique corrélée à une éthique en définissant trois dimensions de l'existence corrélées avec trois genres de connaissance[^principescarto-24] @sec-modeleOntoEthique. Examinons maintenant comment nous définissons de nouveaux principes cartographiques à partir de ces propositions.

[^principescarto-24]: <https://spinoza.fr/les-genres-de-connaissance-extrait-du-cours-de-gilles-deleuze/>
...
Nous nous sommes inspirés des travaux d'Amit Patel[^principescarto-26] pour mettre en place une grille hexagonale et les fonctionnalités nécessaires pour les applications prétopologiques que nous avons codées dans une librairie JavaScript[^principescarto-27] et mises en application dans un module Omeka S (@sec-cartoAffect) afin de gérer les manipulations d'informations dans une base de données.

[^principescarto-26]: Pour une explication des grilles hexagonales : <https://www.redblobgames.com/grids/hexagons/>
    > Pour une proposition d'implémentation algorithmique : <https://www.redblobgames.com/grids/hexagons/implementation.html>

[^principescarto-27]: Le code est accessible ici : <https://github.com/samszo/HDR/docs/jdcCartoHexa.html>
...
Nos principes de cartographie s'accommodent parfaitement de cette modélisation par triplet RDF, mais nous faisons le choix d'un autre type de représentation qui reprend les recommandations du W3C avec le format « Open Annotation »[^principescarto-29] qui code de façon très simple les relations entre des ressources numériques afin de définir un point de vue particulier sur celles-ci. Suivant ce modèle une "annotation" est ce qui met en rapport un "body" à une "target". De notre point de vue, la modélisation du rapport est donc une "annotation" qui dans un ici et maintenant va mettre en relation deux ressources.

[^principescarto-29]: Lien vers les spécifications : <https://www.w3.org/TR/annotation-model/>

![Web Annotation Data Model[^principescarto-30]](media/10000001000002C2000001965EFA665F8AB6D163.png){#fig-webAnnotation fig-align="center" height="200"}

[^principescarto-30]: Lien vers les spécifications : <https://www.w3.org/TR/annotation-model/>
...
Pour gérer ces informations dans la base de données, nous avons utilisé le module Omeka S "Annotation" développé par Daniel Berthereau[^principescarto-32] afin de créer des rapports entre des ressources physiques, actants et concepts. Notons que cette modélisation est elle aussi fractale puisqu’un rapport en tant que ressource peut être en relation avec les autres dimensions existentielles, par exemple pour qualifier le rapport avec un concept particulier, comme ceux proposés par le langage SKOS @fig-cartoHexaAjoutConcept.

[^principescarto-32]: Accessible ici : <https://gitlab.com/Daniel-KM/Omeka-S-module-Annotate>
...
## Modéliser des cribles : vers une cartographie des subjectivités[^principescarto-33] {#sec-modeliserCrible}

[^principescarto-33]: Ce chapitre reprend en les modifiant les chapitres consacrés au crible dans [@szoniecky2020]
...
[^principescarto-38]: <https://www.w3.org/RDF/>

![Semantic Web Stack[^principescarto-39]](media/1000000100000258000002762CF3B23905723B38.png){#fig-semanticWebStack fig-align="center" width="285"}

[^principescarto-39]: Par W3C <https://commons.wikimedia.org/wiki/File:Semantic_Web_Stack.png>
