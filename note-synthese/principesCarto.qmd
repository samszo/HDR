...
[^principescarto-6]:  Le vocubulaire au format rdf turtle : <https://raw.githubusercontent.com/samszo/Omeka-S-module-JDC/master/data/vocabularies/jdc.ttl>

[^principescarto-7]:  Le vocubulaire au format rdf turtle : <https://raw.githubusercontent.com/samszo/Omeka-S-module-JDC/master/data/vocabularies/fup8.ttl>
...
C'est pourquoi nous avons choisi de travailler dans un environnement Web afin de créer dynamiquement des graphiques à partir d'un flux de données et surtout de rendre ces graphiques interactifs. L'autre choix important que nous avons fait est d'utiliser le langage graphique SVG[^principescarto-13] qui permet de manipuler chaque composant graphique de manière autonome [@fry2008]. Ainsi les points, les lignes et les plans disposent d'une autonomie en terme de propriétés graphiques, événementiels et informationnelles. Grâce à la librairie JavaScript D3.js[^principescarto-14] (Data Driven Document) nous pouvons gérer ces propriétés en pilotant les graphiques à partir des données ou à l'inverse les données à partir des graphiques.

[^principescarto-13]:  <https://developer.mozilla.org/en-US/docs/Web/SVG>

[^principescarto-14]:  [https://observablehq.com/\@d3/gallery?utm_source=d3js-org&utm_medium=hero&utm_campaign=try-observable#maps](https://observablehq.com/@d3/gallery?utm_source=d3js-org&utm_medium=hero&utm_campaign=try-observable#maps){.uri}
...
Pour réfléchir sur ces informations les humains ont depuis longtemps développé des systèmes de représentations que ce soit pour le temps [@rosenberg2013; @domenget2017], l'espace [@béguin2017] ou la combinaisons des deux [@serres1997; @giacona2019; @aït-touati2019]. Nous ne rentrerons pas ici dans l'analyse de ces représentations cela dépasserais de loin notre propos qui est de présenter nos principes cartographiques. Nous renvoyons le lecteur curieux à la veille que nous faisons depuis plus de dix ans sur cette question[^principescarto-15].

[^principescarto-15]:  <https://www.diigo.com/user/luckysemiosis?query=%23spatiotempo>
...
: Exemples de projections géographiques[^principescarto-16] {#tbl-projGeo}

[^principescarto-16]:  <https://github.com/d3/d3-geo-projection>

Dans notre cas, pour concevoir des cartes géographique en deux dimensions nous utilisons des librairies JavaScript Open Source comme leaflet.js[^principescarto-17] ou D3.js qui permettent de manipuler des données géographiques modéliser avec le format GeoJSON[^principescarto-18]. Voici par exemple la représentation géographique de mes collaborations dans le monde à partir de mes dépôts dans HAL[^principescarto-19] @sec-datavizActiChercheur :

[^principescarto-17]:  <https://leafletjs.com/>

[^principescarto-18]:  <https://fr.wikipedia.org/wiki/GeoJSON>

[^principescarto-19]:  <https://samszo.github.io/StatsHAL/world.html?q=Samuel%20Szoniecky>
...
Nous suivons sur ce point les principes spinozistes d'une modélisation ontologique corrélée à une éthique en définissant trois dimensions de l'existence corrélées avec trois genres de connaissance[^principescarto-22] @sec-modeleOntoEthique. Examinons maintenant comment nous définissons de nouveaux principes cartographiques à partir de ces propositions.

[^principescarto-22]:  <https://spinoza.fr/les-genres-de-connaissance-extrait-du-cours-de-gilles-deleuze/>
...
Nous nous sommes inspiré des travaux d'Amit Patel[^principescarto-28] pour mettre en place une grille hexagonale et les fonctionnalités nécessaires pour les applications prétopologiques que nous avons codées dans une librairie JavaScript[^principescarto-29] et mises en application dans un module Omeka S (@sec-cartoAffect) afin de gérer les manipulations d'informations dans une base de données.

[^principescarto-28]:  Pour une explication des grilles hexagonales : <https://www.redblobgames.com/grids/hexagons/>\
    \> Pour une proposition d'implémentation algorithmique : <https://www.redblobgames.com/grids/hexagons/implementation.html>

[^principescarto-29]:  Le code est accessible ici : <https://github.com/samszo/HDR/docs/jdcCartoHexa.html>
