...
![Cycle de la sémiose [@µ2015, p. 10]](media/100000010000048C000002D6F38927B016A358A1.png){#fig-cyclesemiose height="200"}
...
![Dynamiques pulsations existentielles](images/localhost_samszo_HDR_pulsationsExistentielles.html.png){#fig-dynamiquesPulsationsExistentielles fig-align="center" height="300"}
...
![Architecture Client / Serveur](media/1000000100000708000005788D09F0CE68E86891.png){#fig-ArchitectureClientServeur height="200"}

Nous ne ferons pas ici une analyse des technologiques de représentation des données [@andry2022; @fekete2015] préférant nous focaliser sur les outils et les méthodes que nous utilisons dans le cadre de ce travail pour cartographier nos connaissances. Nous ne détaillerons pas non plus tous les éléments qui composent notre environnement[^princarto2] mais uniquement les plus pertinents pour comprendre les principes de cartographie que nous avons mis en place dans ce travail pour gérer les données du coté serveur et naviguer dans leurs représentations du coté client.
...
-   4 modules spécifiques pour une gestion spécifique des données dans Omeka S:
    ...
    -   JDC : ce module[^princarto11] fourni les interfaces nécessaires pour modéliser un écosystème de connaissances suivant une ontologie éthique @sec-modeleOntoEthique.
...
[^princarto6]:  Le vocubulaire au format rdf turtle : <https://raw.githubusercontent.com/samszo/Omeka-S-module-JDC/master/data/vocabularies/jdc.ttl>

[^princarto7]:  Le vocubulaire au format rdf turtle : <https://raw.githubusercontent.com/samszo/Omeka-S-module-JDC/master/data/vocabularies/fup8.ttl>
...
[![Coordonnées cartésiennes : planaires](media/100019CC000029720000261A8EDA4DB42B40D384.svg)]{#fig-coor2D fig-align="center" height="200"}(https://commons.wikimedia.org/wiki/File:Rovinna_kartezska_soustava_souradnic.svg)

[![Coordonnées cartésiennes : tridimensionnelles](media/10003C4200002F5F00002EF5D54C4825D5249FE4.svg)]{#fig-coor3D fig-align="center" height="200"}(https://commons.wikimedia.org/wiki/File:Rectangular_coordinates.svg)
...
C'est pourquoi nous avons choisi de travailler dans un environnement Web afin de créer dynamiquement des graphiques à partir d'un flux de données et surtout de rendre ces graphiques interactifs. L'autre choix important que nous avons fait est d'utiliser le langage graphique SVG[^princarto13] qui permet de manipuler chaque composant graphique de manière autonome [@fry2008]. Ainsi les points, les lignes et les plans disposent d'une autonomie en terme de propriétés graphiques, événementiels et informationnelles. Grâce à la librairie JavaScript D3.js[^princarto14] (Data Driven Document) nous pouvons gérer ces propriétés en pilotant les graphiques à partir des données ou à l'inverse les données à partir des graphiques.

[^princarto13]:  <https://developer.mozilla.org/en-US/docs/Web/SVG>

[^princarto14]:  [https://observablehq.com/\@d3/gallery?utm_source=d3js-org&utm_medium=hero&utm_campaign=try-observable#maps](https://observablehq.com/@d3/gallery?utm_source=d3js-org&utm_medium=hero&utm_campaign=try-observable#maps){.uri}
...
Pour réfléchir sur ces informations les humains ont depuis longtemps développé des systèmes de représentations que ce soit pour le temps [@rosenberg2013; @domenget2017], l'espace [@béguin2017] ou la combinaisons des deux [@serres1997; @giacona2019; @aït-touati2019]. Nous ne rentrerons pas ici dans l'analyse de ces représentations cela dépasserais de loin notre propos qui est de présenter nos principes cartographiques. Nous renvoyons le lecteur curieux à la veille que nous faisons depuis plus de dix ans sur cette question[^princarto15].

[^princarto15]:  <https://www.diigo.com/user/luckysemiosis?query=%23spatiotempo>
...
: Exemples de projections géographiques[^princarto16] {#tbl-projGeo}

[^princarto16]:  <https://github.com/d3/d3-geo-projection>

Dans notre cas, pour concevoir des cartes géographique en deux dimensions nous utilisons des librairies JavaScript Open Source comme leaflet.js[^princarto17] ou D3.js qui permettent de manipuler des données géographiques modéliser avec le format GeoJSON[^princarto18]. Voici par exemple la représentation géographique de mes collaborations dans le monde à partir de mes dépôts dans HAL[^princarto19] @sec-datavizActiChercheur :

[^princarto17]:  <https://leafletjs.com/>

[^princarto18]:  <https://fr.wikipedia.org/wiki/GeoJSON>

[^princarto19]:  <https://samszo.github.io/StatsHAL/world.html?q=Samuel%20Szoniecky>

![Répartition des collaborations dans le monde](media/1000000100000391000001C9A8374D4BE530A8B5.png){#fig-collabMondeSamszo fig-align="center" height="200"}
...
Nous nous sommes inspiré des travaux d'Amit Patel[^princarto28] pour mettre en place une grille hexagonale et les fonctionnalités nécessaires pour les applications prétopologiques que nous avons codées dans une librairie JavaScript[^princarto29] et mises en application dans un module Omeka S (@sec-cartoAffect) afin de gérer les manipulations d'informations dans une base de données.

[^princarto28]:  Pour une explication des grilles hexagonales : <https://www.redblobgames.com/grids/hexagons/>\
    \> Pour une proposition d'implémentation algorithmique : <https://www.redblobgames.com/grids/hexagons/implementation.html>

[^princarto29]:  Le code est accessible ici : <https://github.com/samszo/HDR/docs/jdcCartoHexa.html>
...
![Visualisattion des rapports](images/localhost_samszo_HDR_jdcComplexityDiagramme.html.png){#fig-datavizRapport fig-align="center" height="200"}
...
![Réseaux de neurones [@nguyen2018]](images/reseaux-neurones.png){#reseauNeurones fig-align="center" height="200"}
...
![Semantic Web Stack, Par W3C https://commons.wikimedia.org/wiki/File:Semantic_Web_Stack.png](media/1000000100000258000002762CF3B23905723B38.png){#fig-semanticWebStack height="200"}
