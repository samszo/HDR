...
Un environnement Web se base avant tout sur une architecture Client / Serveur qui utilise le protocole HTTP pour organiser les échanges de données entre des machines et des utilisateurs via un navigateur (Balpe et al., 1996) cf. ci-dessous[^hdr-1].
...
-   4 modules spécifiques pour une gestion spécifique des données dans Omeka S:
    ...
    -   Diigo Import : ce module permet d'importer les signets enregistrés dans une base de données Diigo y compris les copies d'écrans[^hdr-6].
    -   Zotero Import Plus : ce module basé augmente le module Zotero Import pour importer les notes prises dans Zotero ainsi que les documents associés aux références bibliographiques[^hdr-7].
    -   JDC : ce module fourni les interfaces nécessaires pour modéliser un écosystème de connaissances suivant une ontologie éthique ([1.5.2](#anchor-18) [Modèle pour une ontologie -- éthique](#anchor-18) p. [51](#anchor-18)) [^hdr-8].
    -   CartoAffect : ce module permet de gérer les données pour la modélisation et la présentation des affects en relation avec un écosystème de connaissances[^hdr-9].
...
Les données de cette environnement Omeka S ont été importées dans la base de données avec les modules d'importation Diigo Import, Zotero Import et Bulk Import[^hdr-10]. Ce dernier module est très pratique pour importer des données à partir de tableurs. Nous avons constitués ces tableurs manuellement par exemple à partir de la version texte de notre CV[^hdr-11] ou automatiquement par exemple avec l'outil que nous avons développée pour extraire les informations des dépôts GitHub[^hdr-12] d'un compte :
...
Les données de l'écosystème de connaissance que nous avons développé pour ce travail représente une base de données SQL de 75 tables peuplées par plus de 2 000 000 de lignes. Le graphique ci-dessous présente la répartition des objets disponibles dans cet écosystème suivant leur classe[^hdr-13] :
...
| Illustration 6: Coordonnées cartésiennes : planaires[^hdr-15]                                        | Illustration 7: Coordonnées cartésiennes tridimensionnelles[^hdr-16]             |
...
Pour réfléchir sur ces informations les humains ont depuis longtemps développé des systèmes de représentations que ce soit pour le temps (Rosenberg & Grafton, 2013) (Domenget et al., 2017), l'espace (Béguin & Pumain, 2017) ou la combinaisons des deux (Serres, 1997) (Giacona et al., 2019) (Aït-Touati et al., 2019). Nous ne rentrerons pas ici dans l'analyse de ces représentations cela dépasserais de loin notre propos qui est de présenter nos principes cartographiques. Nous renvoyons le lecteur curieux à la veille que nous faisons depuis plus de dix ans sur cette question[^hdr-19].
...
Dans notre cas, pour concevoir des cartes géographique en deux dimensions nous utilisons des librairies JavaScript Open Source comme leaflet.js[^hdr-20] ou D3.js qui permettent de manipuler des données géographiques modéliser avec le format GeoJSON[^hdr-21]. Voici par exemple la représentation géographique de mes collaborations dans le monde à partir de mes dépôts dans HAL[^hdr-22] (cf. [2.4.2](#anchor-22) [Représentation de l'activité des chercheurs](#anchor-22) p. [68](#anchor-22)[ ](#anchor))  :
...
Nous nous sommes inspiré des travaux d'Amit Patel[^hdr-31] pour mettre en place une grille hexagonale et les fonctionnalités nécessaires pour les applications prétopologiques que nous avons codé dans une librairie JavaScript[^hdr-32] et mis en application dans un module Omeka S ([3.4.2.2.4](#anchor-28) [CartoAffect](#anchor-28) p. [72](#anchor-28)) afin de gérer les manipulations d'informations dans une base de données.
...
En d'autres termes, nous reprenons ici les principes que recommande le W3C avec le format « Open Annotation »[^hdr-33] qui code de façon très simple les relations entre des ressources sémantiques afin de définir un point de vue particulier sur celles-ci. Pour gérer, ces informations dans la base de données nous avons utilisé le module Omeka S développé par Daniel Berthereau[^hdr-34].
...
Cette copie d'écran montre les parties de l'article que nous avons sélectionnéses (marge jaune) et les notes que nous avons prises pour la sélection (marge grise). En ce référent à cette page d'annotation, il est pratique de retrouver rapidement ce qui nous a semblé pertinent et pourquoi[^hdr-44].
...
Un autre usage particulièrement intéressant des annotations est la conservation de références qui ne sont plus accessibles en ligne et qui représente plus de 10 % des URL (cf. ci-dessous). Lorsque nous importons les annotations Web depuis Diigo vers notre de base de connaissances Omeka S (cf. [3.4.2.2.1Importation des données de diigo](#anchor-33) p. [72](#anchor-33)), nous testons la validité de l'URL et enregistrons son statut[^hdr-45] ce qui permet de savoir quelles URL sont obsolètes :
...
Ce travail d'annotation et de sélection de citation nous fourni une base de connaissances de plus de 1 400 références bibliographiques et plus de 19 000 références Web qui sont indexées par plus de 6 000 concepts. L'ensemble de ces données sont accessibles soit sur les Zotero pour les références bibliographiques[^hdr-46], soit sur Diigo pour les références Web[^hdr-47], soit directement sur notre base Omeka S dans un format HTML pour naviguer dans la base de données ou en JSON[^hdr-48] pour analyser les données avec des algorithmes.
...
### La complexité existentielle[^hdr-49]
...
-   infos
    ...
    -   resources[^hdr-52] : liste des ressources impliquée dans le calcul avec les informations :
...
Développer spécifiquement pour ce travail d'HDR afin d'extraire les données d'un compte GitHub et les importer dans Omeka S, cet outil cumule les besoins de deux outils existant : githubUserMeta[^hdr-54] et githubScraper[^hdr-55].
