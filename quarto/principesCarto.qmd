...
Nous nous sommes inspiré des travaux d'Amit Patel[^principescarto-33] pour mettre en place une grille hexagonale et les fonctionnalités nécessaires pour les applications prétopologiques que nous avons codé dans une librairie JavaScript[^principescarto-34] et mis en application dans un module Omeka S (@sec-cartoAffect) afin de gérer les manipulations d'informations dans une base de données.
...
En d'autres termes, nous reprenons ici les principes que recommande le W3C avec le format « Open Annotation »[^principescarto-35] qui code de façon très simple les relations entre des ressources sémantiques afin de définir un point de vue particulier sur celles-ci. Pour gérer, ces informations dans la base de données nous avons utilisé le module Omeka S développé par Daniel Berthereau[^principescarto-36].
